...
for i in range(1 , N):       
    answerArr[0][i] = answerArr[i - 1][0] + table[i][0]
    answerArr[i][0] = answerArr[0][i - 1] + table[0][i]
...
for x1, y1, x2, y2 in targetXy:
    ...
    if x1 == 1 and y1 > 1:
        answer = answerArr[y2-1][x2-1] - answerArr[y1- 2][x2 - 1]
        
        
    if x1 > 1 and y1 == 1:
        answer = answerArr[y2-1][x2-1] - answerArr[y2- 1][x1 - 2]

    if x1 ==1 and y1 == 1:
        answer = answerArr[y2-1][x2-1]
    print(answer)
